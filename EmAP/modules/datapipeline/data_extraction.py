...
class DataCollection():
    def process_video(self,input_dir, output_dir, file_name , splitval = "5"):
        """
        ...
        for i in splitvideolist:
            ...
            thread = threading.Thread(target=self.extract_frames, args=(inputfile, frame_output_template))
            thread.start()
            threads.append(thread)

            # Extract audio in a new thread
            outputfile = os.path.join(splitaudiosrc, i.split(".")[0] + ".wav")
            thread = threading.Thread(target=self.extract_audio, args=(inputfile, outputfile))
            thread.start()
            threads.append(thread)
        return new_dir, splitframessrc, splitaudiosrc, splitvideosrc
    def extract_frames(self,input_file, output_template):
        """
    ...
    def extract_audio(self,input_file, output_file):
        ...
        subprocess.run(["ffmpeg", "-i", input_file, "-vn", "-acodec", "pcm_s16le", "-ar", "48000", "-ac", "1", output_file])
        # subprocess.run(["ffmpeg", "-i", input_file, "-vn", "-acodec", "libmp3lame", output_file])

    

    def extract_entire_text(self,splitaudiosrc):
        """
        splitaudiosrc: path
        """
